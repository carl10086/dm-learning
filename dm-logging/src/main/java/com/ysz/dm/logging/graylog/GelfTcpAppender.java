...
import com.ysz.dm.logging.graylog.pool.SimpleObjectPool;
...
public class GelfTcpAppender extends AbstractGelfAppender {
   ...
   * Default: {@value DEFAULT_CONNECT_TIMEOUT} milliseconds.
   */
  private int connectTimeout = DEFAULT_CONNECT_TIMEOUT;

  /**
   * Maximum time (in milliseconds) to block when reading a socket. A value of 0 disables the socket timeout. Default:
   * {@value DEFAULT_SOCKET_TIMEOUT} milliseconds.
   */
  private int socketTimeout = DEFAULT_SOCKET_TIMEOUT;

  /**
   * Time interval (in seconds) after an existing connection is closed and re-opened. A value of -1 disables automatic
   * reconnects. Default: {@value DEFAULT_RECONNECT_INTERVAL} seconds.
   */
  ...
  /**
   * Time (in milliseconds) between retry attempts. Ignored if maxRetries is 0. Default: {@value DEFAULT_RETRY_DELAY}
   * milliseconds.
   */
  ...
  /**
   * Maximum amount of time (in milliseconds) to wait for a connection to become available from the pool. A value of -1
   * disables the timeout. Default: {@value DEFAULT_POOL_MAX_WAIT_TIME} milliseconds.
   */
  private int poolMaxWaitTime = DEFAULT_POOL_MAX_WAIT_TIME;

  /**
   * Maximum amount of time (in seconds) that a pooled connection can be idle before it is considered 'stale' and will
   * not be reused. A value of -1 disables the max idle time feature. Default: {@value DEFAULT_POOL_MAX_IDLE_TIME}.
   */
