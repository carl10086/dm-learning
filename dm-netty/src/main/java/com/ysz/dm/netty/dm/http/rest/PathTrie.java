...
public class PathTrie<T> {
  ...
  public class TrieNode {
        ...
        if (index == (path.length - 1)) {
          if (node.value != null) {
            throw new IllegalArgumentException(
                "Path [" + String.join("/", path) + "] already has a value ["
                    + node.value + "]");
          } else {
            node.value = value;
          }
    ...
    public synchronized void insertOrUpdate(String[] path, int index, T value,
    ...
    public T retrieve(String[] path, int index, Map<String, String> params,
        TrieMatchingMode trieMatchingMode) {
      if (index >= path.length) {
        return null;
      }
      ...
      if (node == null) {
        if (trieMatchingMode == TrieMatchingMode.WILDCARD_NODES_ALLOWED) {
          ...
          usedWildcard = true;
        } else if (trieMatchingMode == TrieMatchingMode.WILDCARD_LEAF_NODES_ALLOWED
            && index + 1 == path.length) {
          /*
      ...
      if (nodeValue == null && !usedWildcard
