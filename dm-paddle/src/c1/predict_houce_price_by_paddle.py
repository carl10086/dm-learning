...
for epoch_id in range(EPOCH_NUM):
    ...
    test_data_features = paddle.to_tensor(test_data[:, :-1])
    test_date_prices = paddle.to_tensor(test_data[:, -1:])

    test_data_predict = model(test_data_features)

    # error = test_data_predict - test_date_prices
    # loss = paddle.tensor.sum(error * error) / error.shape[0]
    #
    # print(f"loss:{loss}")
